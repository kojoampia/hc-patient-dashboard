...
- Local API traffic is proxied by `webpack/proxy.conf.js` to `http://localhost:5505`. The gateway's dev port is `5503` and `docker-compose.yml` uses `5501` — verify which one is live before debugging API failures.
