...
**`fakeAsync` cannot test `async`/`await` here, and it fails by making the code under test look broken** (2026-09-18, `docs/backlog.md` item 39 cycle 2). `tsconfig.json:19` targets **`es2022`**, so an `async` function compiles to a native one — and zone.js patches the global `Promise`, not the engine's own await machinery, so it never sees a native `await`. Inside a synchronous `fakeAsync` body the continuation after `await fetch(...)` therefore **never runs at all**: `tick()` has nothing to flush, the code appears never to have connected, opened, or resolved, and every assertion past the first `await` is vacuous. The first draft of `membership-stream.service.spec.ts` failed **9 of 16** this way, with messages naming the product (`Expected 2, received 1`) rather than the harness. Use an `async` test with Jest's own fake timers — `jest.useFakeTimers()` plus `await jest.advanceTimersByTimeAsync(ms)` — which drains native microtasks between timers. `fakeAsync` remains correct for Observable- and `setTimeout`-based code, which is most of this repo and why it is the house idiom everywhere else.
...
CI runs `prettier:check`, lint, tests and `webapp:prod` — `prettier:check` added 2026-09-09. It failed on every push between 2026-07-30 and 2026-08-03 and has not failed since — 84 runs, checked 2026-08-31. **The filename is vestigial**: the workflow no longer publishes anything, and says so in its own header.

**It does not, however, _gate_ — this entry said "CI does gate" and that was wrong** (corrected 2026-09-09). The repository's single ruleset carries `pull_request`, `deletion` and `non_fast_forward` and **no `required_status_checks`**, with `required_approving_review_count: 0`, so a red run blocks nothing and the author can merge their own PR. `mobile` is the same, so this is an estate-wide setting rather than this repo's oversight; it is filed as its own backlog item. Until it changes, every check here is **visible, not binding**.
...
- Indentation is 2 spaces everywhere (`.editorconfig` root `indent_size = 2`; its `[*.md]` section only disables trailing-whitespace trimming). **Prettier does not run on commit, whatever `package.json` implies** — corrected 2026-09-09, `docs/backlog.md` item 20. `prepare: husky install` and the two `.lintstagedrc` files (`.cjs` and `.js`, with different globs) describe a hook that does not exist: `.husky/` holds only `_/`, has no `pre-commit`, and is untracked. This entry claimed the opposite while **283 files** drifted from the pinned prettier 3.1.0 with nothing to say so. `prettier:check` is now a CI step; the hook is still absent, deliberately left for its own decision.
