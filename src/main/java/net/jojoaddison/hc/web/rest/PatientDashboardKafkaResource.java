...
@RestController
@RequestMapping("/api/patient-dashboard-kafka")
public class PatientDashboardKafkaResource {

    private final Logger log = LoggerFactory.getLogger(PatientDashboardKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public PatientDashboardKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
