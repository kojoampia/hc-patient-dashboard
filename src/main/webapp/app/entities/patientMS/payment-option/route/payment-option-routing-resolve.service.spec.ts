...
import paymentOptionResolve from './payment-option-routing-resolve.service';

describe('PaymentOption routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: PaymentOptionService;
  let resultPaymentOption: IPaymentOption | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IPaymentOption returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        paymentOptionResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
      ...
      expect(resultPaymentOption).toEqual({ id: 'ABC' });
    });
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultPaymentOption).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IPaymentOption>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 'ABC' };
