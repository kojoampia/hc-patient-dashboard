...
import net.jojoaddison.hc.config.EmbeddedKafka;
import net.jojoaddison.hc.config.KafkaSseConsumer;
import net.jojoaddison.hc.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class PatientDashboardKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/patient-dashboard-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/patient-dashboard-kafka/consume")
